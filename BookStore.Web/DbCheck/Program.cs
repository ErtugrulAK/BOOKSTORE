...
using (var conn = new NpgsqlConnection(connString))
{
    ...
    // Mevcutları temizle ve ID'leri 1'den başlat
    using (var cleanCmd = new NpgsqlCommand("TRUNCATE \"Books\", \"Users\", \"Orders\", \"OrderItems\", \"ContactMessages\", \"UserAddresses\" RESTART IDENTITY CASCADE;", conn)) { cleanCmd.ExecuteNonQuery(); }
    ...
    // Admin'i İLK SIRADA ekle (ID: 1 olması için)
    var adminCmd = new NpgsqlCommand(@"INSERT INTO ""Users"" 
        (""Username"", ""PasswordHash"", ""Role"", ""FirstName"", ""LastName"", ""Email"", ""CreatedAt"", ""PhoneNumber"") 
        VALUES (@user, @hash, @role, @f, @l, @email, @date, @phone)", conn);
    ...
    adminCmd.Parameters.AddWithValue("l", "");
    adminCmd.Parameters.AddWithValue("email", "admin");
    adminCmd.Parameters.AddWithValue("date", DateTime.UtcNow);
    adminCmd.Parameters.AddWithValue("phone", "");
    ...
    Console.WriteLine("1 Admin (ID:1) ve 25 kullanıcı eklendi.");
}
